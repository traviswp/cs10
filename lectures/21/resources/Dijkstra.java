 ...
 *
 * <p>
 * To execute the algorithm, use the {@link #execute(Graph,Vertex,Object)
 * execute} method, and then make subsequent calls to the
 * {@link #getDist(Vertex) getDist} method to obtain the shortest distance from
...
public class Dijkstra<V, E> {
    ...
    public void execute(Graph<V, E> g, Vertex<V> s, Object w) {
        graph = g;
        WEIGHT = w;
        DefaultComparator<Object> dc = new DefaultComparator<Object>();
        Q = new HeapAdaptablePriorityQueue<Integer, Vertex<V>>(dc);
        dijkstraVisit(s);
    }

    /**
     * Get the distance of a vertex from the source vertex. This method returns
     * the length of a shortest path from the source to <tt>u</tt> after
     * {@link #execute(Graph,Vertex,Object) execute} has been called.
     * 
     * @param u
